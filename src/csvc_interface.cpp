...
double csvc_interface::parameter_comparison(const vvvector<double>& vSig, const vvvector<double>& vBkg, const vvvector<double>& VarvSig, const vvvector<double>& VarvBkg, const int iteration, const double pre_accur, double C){
  ...
  if( ( ( accuracy->at(highest) > ( pre_accur * dOneSig ) ) || ( C < 0.6 && accuracy->at(highest) > ( pre_accur * dPreAccFlucAllowRatio )) ) && iteration < max_iter && plato_counter < 5 ){
    ...
    cout <<" For given C value: "      << std::setprecision(iPrecisionV) << C                        << 
      "  highest significance: "           << std::setprecision(iPrecisionC)  << accuracy->at(highest)    << 
      "  obtained with gamma: "        << std::setprecision(iPrecisionV) << gamma [highest]          << 
      "  on the bin "                  << std::setprecision(iPrecisionC)  << max_cut_bin->at(highest) << 
      "  in the iteration "            << iteration             << endl;
    overall_accuracy.push_back(high_accuracy_settings);
    if(iteration == max_iter - iPreciseScanSteps && precise_scan) { 
      parameter_decision();
    } else { 
...
bool csvc_interface::split_set_sample (const svm_container & svm){
  ...
  min_signal   = nsig_test_w * dMinEff; 
